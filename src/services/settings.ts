...
export async function fetchSettings(tenantId: string, branchId: string) {
  return supabase.from('settings').select('*').eq('tenant_id', tenantId).eq('branch_id', branchId);
}

export async function upsertSetting(tenantId: string, branchId: string, key: SettingKey, value: string) {
  return supabase
    .from('settings')
